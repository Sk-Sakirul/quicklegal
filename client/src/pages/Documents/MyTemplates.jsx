...
import { fetchUserDocuments } from "../../features/documentSlice";

const MyTemplates = () => {
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const { userDocuments, loading, error } = useSelector(
    (state) => state.document
  );

  useEffect(() => {
    if (user?._id) {
      dispatch(fetchUserDocuments(user._id)); // fetch only user's docs
    }
  }, [dispatch, user]);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      <h2 className="text-2xl font-bold text-indigo-700 mb-4">My Templates</h2>
